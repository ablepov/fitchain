...
1. [x] Мини-график последних подходов (SVG/Canvas без тяжёлых зависимостей) — компонент `MiniChart` уже присутствует в кодовой базе.
2. [x] `SummaryPanel`: базовая сводка за день уже присутствует в кодовой базе; качество и полноту ещё нужно отдельно перепроверить.
3. [ ] Эмоциональная подсказка: время с последнего подхода. [MCP Playwright: базовые проверки сводки]

> Примечание: план частично отстаёт от фактического состояния репозитория. Перед новыми этапами сверяемся не только с этим файлом, но и с `CURRENT_STATE.md`.
